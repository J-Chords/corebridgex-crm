...
export interface AdminCreateUserResult {
  user: AdminUserRow;
  /**
   * CD-162 post-manual-QA pass — a cryptographically secure temporary password, generated by the
   * provider (never invented/typed by the Admin), returned exactly once at creation time so the
   * Admin can hand it to the new user. The account is created with `mustChangePassword: true`, so
   * this value stops being useful the instant the user signs in and sets their own — it is never
   * re-fetchable afterward, matching `resetPassword`'s own "Admin can never retrieve it again" rule.
   */
...
export interface AdminUsersProvider {
  ...
  createUser(viewer: User, input: AdminCreateUserInput): Promise<AdminCreateUserResult>;
