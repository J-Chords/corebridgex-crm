...
export default function ClientReportDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canEditWording = canEditClientReportWording(user, report) && !isTrashed;
  const canComment = hasReportingReviewAccess(user) ? !isOwner : !isOwner && !isEmployee(user);
  const canFinalize = canFinalizeClientReport(user, report) && !isTrashed;
  const canTrash = canTrashClientReport(user, report);
  const canRestore = canRestoreClientReport(user, report);
  const usingDraft = (canEditEntries || canEditWording) && draft;
  const departments = usingDraft ? draft : report.departments;
  ...
  function wordingEditsFromDraft(current: ClientReportDepartmentSection[]) {
    return current.flatMap((dept) => dept.activities.flatMap((activity) => activity.lineItems.map((item) => ({ id: item.id, details: item.details }))));
  }
  ...
  async function handleSave() {
    ...
    try {
      if (isOwner) {
        ...
        // A reporting reviewer editing someone else's draft only ever gets the wording-only lane
        // (`canEditClientReportWording`) — sending every line's current `details` is idempotent for
        // lines the reviewer didn't touch, and the server-side RPC/mock only ever applies the
        // `details` field regardless, so factual fields can never be smuggled through this path.
        await clientReportProvider.updateDraftWording(user, id, wordingEditsFromDraft(draft));
      }
      setIsDirty(false);
  ...
  async function handleFinalize() {
    ...
    try {
      // Save any pending edit first, through whichever lane applies (owner's full edit, or a
      // reviewer's wording-only edit) — a Superadmin/reviewer finalizing someone else's draft with
      // no edit access of their own skips straight to finalizeReport (see canFinalizeClientReport).
      if (draft && isDirty) {
        if (isOwner) {
          await clientReportProvider.updateDraft(user, id, draft);
        } else if (canEditWording) {
          await clientReportProvider.updateDraftWording(user, id, wordingEditsFromDraft(draft));
        }
      }
          ...
          <div className="flex flex-col gap-2">
            ...
            {!isOwner && !isTrashed && canEditWording && (
              <p className="text-xs text-muted-foreground">
                You&apos;re reviewing a report generated by someone else — as a reporting reviewer you can correct
                wording and finalize it, but Task/Service/Activity/date/duration stay factual truth from the
                operational source.
              </p>
            )}
            {!isOwner && !isTrashed && !canEditWording && (
              <p className="text-xs text-muted-foreground">
                ...
                they (or a reporting reviewer) can edit its entries.
              </p>
            )}
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[240px_1fr] print:block">
        ...
        <div className="flex flex-col gap-4">
          ...
          <ClientReportView
            ...
            dailyVisitMinutes={report.dailyVisitMinutes}
            editable={canEditEntries}
            wordingEditable={!canEditEntries && canEditWording}
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 print:hidden">
        {isTrashed ? (
          ...
          <>
            ...
            {(canEditEntries || canEditWording) && (
              <Button variant="outline" disabled={isSaving} onClick={handleSave}>
                {isSaving ? "Saving…" : canEditEntries ? "Save changes" : "Save wording"}
              </Button>
            )}
