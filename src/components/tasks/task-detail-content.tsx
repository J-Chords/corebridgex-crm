...
import { TaskLegacyHistorySection } from "@/components/tasks/task-legacy-history-section";
 ...
 * Checklist, legacy Handoff/Notes history (Task Level Phase 2 — collapsed, only when real history
 * exists), Comments, then Time Activity last — Time Tracking's primary controls live exclusively in
 * the page's right property rail now, so this column never repeats them (Part 34's locked order,
 * addressing the "Time Tracking dominates the page" criticism directly). Status/Priority/Due date/
 * Assignees/Estimated/Tracked/Created By all live in the right rail too — this column is purely "the
 * place the work happens," not a second copy of the header's own metadata. Each embedded section
 * still owns its own data fetching; the ones with their own Card chrome keep it, the lighter ones
 * (Description, Checklist, Time Activity) use a plain heading + divider instead of a nested card, per
 * the "whitespace over card-everything" rule.
 */
export function TaskDetailContent({ task, onChanged, timer }: TaskDetailContentProps) {
  const { user } = useAuth();
  const { handoffs } = useTaskHandoffs(task.id);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <TaskLegacyHistorySection taskId={task.id} handoffs={handoffs} notes={notes} />
