...
**Current:**
```
cb2e7c5ff57dae46d298bcc69e515ef156ae27ae
checkpoint: employee and supervisor operational ux
2026-08-14
Phase 6 — Employee & Supervisor Operational UX. Manually accepted by the user.
```

**Previous:**
```
dc92141059077d07bc28a8ff9396ce8ff3782d11
checkpoint: service activity task domain alignment
```

**Uncommitted on top of the current checkpoint (Phase 7 — Core Operational Supabase Backend)**: 6 new migrations (workstreams/activity-catalog/tasks/time-entries schema + a function-grant hardening fix + demo seed, all pushed to the hosted `corebridgex-crm` project) and 5 real Supabase business providers (Companies/Workstreams/ActivityCatalog/Tasks/TimeEntries) plus a minimum real Notifications provider, a new `supabase-core` provider mode, and a small `notes-section.tsx` error-handling fix. Deliberately **not yet committed** — pending one rapid manual real-backend smoke test. See "Phase 7" under "Current phase status" below for the full detail.
...
15. **Phase 8A — Project / Annual Contract Foundation** — implemented, uncommitted, pending manual acceptance. Real `projects`/`project_members` tables, non-destructive backfill, read-only `/dashboard/projects` + `/dashboard/projects/[id]`.
16. Phase 8B — Project workspace (tabbed) + role-aware navigation + Employee "+ Add Service" UI + Task Center Employee access — not started.
...
- Date: 2026-08-14
- Date: 2026-08-15
- Latest committed checkpoint: **`c63e86d03fafeb516846bb65759cfdacf19ae836`** ("checkpoint: complete Supabase business backend") — the full Phase 7 (7A-7E + hotfixes) real Supabase business backend, manually accepted by the user. Previous: `cb2e7c5ff57dae46d298bcc69e515ef156ae27ae`.
- Current phase: **Phase 8A — Project / Annual Contract Foundation** (see the "Phase 8" section above for full detail). New `projects`/`project_members` tables, RLS, a non-destructive/non-fabricating backfill (one Project per existing Company, contract dates copied only when they already existed, Internal Company gets null dates), and a read-only `ProjectsProvider` + `/dashboard/projects`/`/dashboard/projects/[id]` routes (not yet linked from the sidebar). `tsc`/`eslint`/`build` clean in all four modes. **Deliberately uncommitted** — pending manual acceptance.
- Next after that acceptance: **Phase 8B** — Project workspace (tabbed) + role-aware navigation + Employee "+ Add Service" UI + Task Center Employee access.
