...
**Phase 7 final acceptance remains pending** — all of Phase 7A through 7E, the Phase 7A-C hotfix, and this final-acceptance hotfix are uncommitted, awaiting one real end-to-end manual test by the user before checkpointing together.
...
13. ~~Phase 7E — Reports + Productivity~~ (Accomplishments Reports/Client Reports real; assignee-scope hardening resolved) — done. Full `supabase` mode now selects a real implementation for all 14 provider groups. **All of Phase 7 (7A-7E + hotfix) is uncommitted, pending one final manual full-Supabase smoke test.**
14. Real Auth/Postgres/RLS migration is now functionally complete across every current feature. Remaining future work is additive (sub-tasks, client long-lived notes, batch task creation, attendance/geolocation) — not another backend migration slice.
15. Sub-tasks — one nesting level (real child Task: own status/assignee/time/checklist). `tasks.parent_task_id` will be reserved (nullable, unused) in the Tasks migration when reached.
16. Client long-lived/forever note + Client activity/history/reporting.
17. Batch Task creation from multiple Activities at once (workflow improvement, not a data-model change).
18. Attendance/clock-in/geolocation — future only, not scoped now.
...
- Date: 2026-08-14
- Latest committed checkpoint: `cb2e7c5ff57dae46d298bcc69e515ef156ae27ae` ("checkpoint: employee and supervisor operational ux"). Previous: `dc92141059077d07bc28a8ff9396ce8ff3782d11`.
- Current phase: **Phase 7 (7A-7C + hotfix + 7D + 7E) — the full Supabase business backend migration is now functionally complete.** All 14 provider groups (Auth, Companies, Workstreams, Activity Catalog, Tasks, Time Entries, Notifications, Notes, Task Handoffs, Saved Views, Templates, Daily Updates, Accomplishments Reports, Client Reports) have real Supabase implementations. `NEXT_PUBLIC_DATA_PROVIDER=supabase` now selects a genuinely complete real backend with zero `notImplemented` stubs. `supabase-core` (the transitional mode used for `.env.local` during this phase) is unchanged. `tsc`/`eslint`/`build` are clean in all four modes (`mock`/`supabase-auth`/`supabase-core`/`supabase`), each run as an ephemeral override without touching `.env.local`. The Phase 7A-C-disclosed assignee-scope-hardening limitation is now resolved (a small trigger reusing the existing `manages_user()` helper). One data gap remains for the project owner to close manually whenever wanted: 3 of 4 real development profiles have an empty `full_name` (fixable via the same SQL Editor mechanism as the role bootstrap — this process doesn't know the real names and won't guess them). **Deliberately uncommitted** — pending one final manual full-Supabase smoke test (see the phase report's exact steps), after which all of Phase 7 checkpoints together as one commit.
- **Final-acceptance hotfix applied on top of the above**: fixed a real-mode-only relation-hydration bug (Task creator / Workstream lead resolution — see "Phase 7 final-acceptance hotfix" above) via a new narrow `resolve_profile_directory` RPC, and implemented the boss's new rule that an Employee may create a Workstream (choosing Service + Activities) and their own Tasks under it, for a Company they can already access — team assignment and later editing stay Supervisor/Superadmin-only, and Company/contact administration is untouched. No UI entry point was added for this yet (deferred to the next phase). Still uncommitted, still pending final manual acceptance.
- Next after that checkpoint: a Zoho-inspired Project/Task UX realignment (including reworking the Employee-facing Company view into a more operational, less admin-styled workspace) — not started, not designed yet. After that, remaining roadmap items (sub-tasks, client long-lived notes, batch task creation, attendance/geolocation) are additive features, not more Supabase backend migration work.
