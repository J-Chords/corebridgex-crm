...
**Next planned slice: Phase 8C** — Task Center polish + Task drawer.
...
16. **Phase 8B — Employee-First Project Workspace** — implemented, uncommitted, pending manual acceptance. Role-aware navigation, tabbed Project workspace, Employee "+ Add Service"/"+ Add Task", Project-aware Workstream creation, Task Center open to Employees.
17. Phase 8C — Task Center polish + Task drawer. Phase 8D — Planner. Phase 8E — Dashboard expandable cards + Project creation polish. Phase 9 — Subtasks. Then: Client long-lived/forever note (belongs on Company, per the Phase 8 audit), Timesheets (deferred, not yet justified), attendance/geolocation (future only, not scoped now).
...
- Date: 2026-08-15
- Date: 2026-08-17
- Latest committed checkpoint: **`9b5a2d8789f9ea2909b2e37bbb7b368e656c68ee`** ("checkpoint: project annual contract foundation") — Phase 8A, manually accepted by the user. Previous: `c63e86d03fafeb516846bb65759cfdacf19ae836`.
- Current phase: **Phase 8B — Employee-First Project Workspace** (see the "Phase 8B" section above for full detail). Role-aware navigation (Projects for every role, Companies Superadmin-only, Tasks open to everyone), a real tabbed Project workspace, Employee "+ Add Service"/"+ Add Task", `Workstream.projectId` + an `enforce_workstream_project_link` trigger closing every creation call site, atomic `create_workstream`/`create_task` RPCs (closing a real RLS bootstrap bug found in manual testing) now further hardened for Supervisor's team-only scope, Client Contacts narrowed to Superadmin-only, and the real Activity Catalog expanded from the mock source of truth. `tsc`/`eslint`/`build` clean in all four modes; a full rollback-safe role-simulated security probe suite (Employee/Supervisor/Superadmin/Contacts) passed before this pass's migrations were pushed. **Deliberately uncommitted** — pending one combined manual acceptance test.
- Next after that acceptance: **Phase 8C** — Task Center polish + Task drawer.
