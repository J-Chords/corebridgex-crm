...
## Task Level — Phase 2 status (see `docs/current-project-state.md`'s own "Task Level — Phase 2"
section for the full write-up)

**Phase 2 (Product UX / Cross-App Integration) is now IMPLEMENTED, uncommitted, CORE ACCEPTANCE
PENDING.** Presentation/UX-only, built on the Phase 1 checkpoint — no schema or security change.
Task detail's context/history/time/Properties presentation was corrected in place (global Service
name primary, one collapsed Legacy History section instead of an always-present empty Handoff card,
one Tracked-time source of truth instead of two); Estimated Time is now exposed in Create/Edit Task
via the existing `ExpectedTimeInput`; the Global Tasks page and the Project Tasks tab both now expose
the same three views (List/Board/Timeline), reusing the same `TaskBoard`/`TaskTimeline` components;
the Section 24 "full sweep" of Service identity (global name primary, Project-Service qualifier
secondary) referenced above is now done, cross-app. A genuine, previously-unreported CSS-specificity
bug was found and fixed in the shared `Sheet` primitive that was silently narrowing the Project Edit
drawer regardless of its own `sm:max-w-4xl` className; New and Edit Project already shared one
